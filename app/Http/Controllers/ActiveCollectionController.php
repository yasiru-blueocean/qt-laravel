...
use App\Models\Sale;
use App\Http\Resources\ActiveCollectionResource;

class ActiveCollectionController extends Controller
{
    public function index(Request $request)
    {
        $query = Sale::with(['unit.project.company', 'customer', 'handlingPerson'])
    ...
    $query->whereHas('unit.project', function ($q) use ($request) {
        $q->where('project_id', $request->formSelectedProject);
    });
}
        ...
        return ActiveCollectionResource::collection($sales);
    }
}